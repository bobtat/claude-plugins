...
| Plugin | Description |
| ------ | ----------- |
| [accomplishments](plugins/accomplishments) | Builds the evidence record a performance review needs before the evidence is gone — a silent session hook that distils each transcript into a redacted digest of your own prompts before Claude Code's 30-day purge deletes it, a sweep that mines git, PRs, and reviews given and then interviews the gaps, and authoring that turns dated entries into a self-assessment, promotion packet, or resume bullets without inventing a number |
| [agentic-qa](plugins/agentic-qa) | Runs a live, human-in-the-loop QA walkthrough of finished functionality against a ticket's acceptance criteria and its merged PR — a browser, an API, a CLI — with a blast-radius critic that reads the diff, a hard split between contained and escaping irreversible actions, agent-invoked operation with an escalation mechanism instead of guessing, and a report with the screenshots embedded, not just linked |
