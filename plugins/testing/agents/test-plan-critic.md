...
1. **Oracle contamination.** Any expected value in the plan that came from the implementation rather than the description. Highest severity: these produce tests that cannot fail. Symptoms — a precise expected value the spec never mentions; a case phrased in the implementation's vocabulary; an "obvious" default nobody stated. Contamination also arrives *declared*: a case resting on an `observed` behavior is legitimate only as a blocked question or an explicitly labeled characterization case. Treat an `observed` behavior planned as an ordinary correctness test as a finding at this severity, and check the spec's `stated` behaviors for outcomes that look read rather than quoted — a mislabeled one is the same defect with the declaration removed.
2. **Fabricated requirements.** Cases with no behavior ID, or resting on an `inferred` or `observed` behavior the user has not confirmed. Being a good idea does not make it a requirement, and neither does being what the code already does.
