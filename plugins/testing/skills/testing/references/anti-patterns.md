...
**On the "green for the wrong reason" mechanism, be precise about the framework.** A broad exception assertion swallowing an unrelated failure is real, but it depends on the assertion matching *subtypes*: `pytest.raises(Exception)`, JUnit's `assertThrows(Exception.class, …)`, and Jest's bare `.toThrow()` all behave that way. **xUnit's `Assert.Throws<T>` does not** — it matches the exact type and fails on a derived exception; NUnit's `Assert.Throws<T>` and MSTest's `Assert.ThrowsException<T>` are exact as well. The C# forms that carry the hazard are `Assert.ThrowsAny<Exception>` (xUnit) and `Assert.Catch<Exception>` (NUnit). `Assert.Throws<Exception>` is still worth flagging as over-broad — it pins a bare `throw new Exception()` — but not on this reasoning.
