...
- **Oval and pear:** X = 0.625. **Marquise, baguette:** 0.5. **Emerald:** 0.714. All of
  ...
  width correction for sizes of about 5–20 mm (`operators/gem/gem_ratio.py`).
...
`gem_edit(use_force=True)` **replaces the mesh** with JewelCraft's gem for `cut`
(ROUND/DIAMOND if not given), scaled to the object's Y dimension, and renames the object.
...
- It's a volume estimate (cone/pyramid/prism × a per-cut correction × stone density),
  so it can differ slightly from the round-diamond formula (6.5 mm round: 1.004 vs
  1.002 ct).
